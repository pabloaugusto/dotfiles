...
#################################################################################
	...
	# remove deprecated .sops symlink from old layout if it points to dotfiles\app\df\sops
	...
	if (Test-Path -Path $legacySopsLink) {
		...
		if ($null -ne $legacySopsItem -and $legacySopsItem.LinkType -eq 'SymbolicLink' -and "$($legacySopsItem.Target)" -like "*\dotfiles\app\df\sops*") {
	...
	Add-Symlink "$Env:USERPROFILE\.config\git" "$DotFilesDirectory\app\df\git" > $null
	Add-Symlink "$Env:USERPROFILE\.gitconfig-windows" "$DotFilesDirectory\app\df\git\.gitconfig-windows" > $null
	Add-Symlink "$Env:USERPROFILE\.gitconfig.local.sample" "$DotFilesDirectory\app\df\git\.gitconfig.local.sample" > $null
	Add-Symlink "$Env:USERPROFILE\.bashrc" "$DotFilesDirectory\app\df\bash\.bashrc" > $null
	Add-Symlink "$Env:USERPROFILE\.bash_profile" "$DotFilesDirectory\app\df\bash\.bash_profile" > $null
	Add-Symlink "$Env:USERPROFILE\.bashrc_profile" "$DotFilesDirectory\app\df\bash\.bashrc_profile" > $null
	Add-Symlink "$Env:USERPROFILE\.config\winfetch" "$DotFilesDirectory\app\df\winfetch" > $null
	...
	Add-Symlink "${mydocs}\Powershell\profile.ps1" "$DotFilesDirectory\app\df\powershell\profile.ps1" > $null
	Add-Symlink "${mydocs}\Powershell\Microsoft.PowerShell_profile.ps1" "$DotFilesDirectory\app\df\powershell\Microsoft.PowerShell_profile.ps1" > $null
	Add-Symlink "${mydocs}\WindowsPowerShell\Microsoft.PowerShell_profile.ps1" "$DotFilesDirectory\app\df\powershell\Microsoft.PowerShell_profile.ps1" > $null
	Add-Symlink "${mydocs}\powershell\env-vars.ps1" "$DotFilesDirectory\app\df\powershell\env-vars.ps1" > $null
	Add-Symlink "${mydocs}\powershell\env-check.ps1" "$DotFilesDirectory\app\df\powershell\env-check.ps1" > $null
	Add-Symlink "${mydocs}\powershell\plugins.ps1" "$DotFilesDirectory\app\df\powershell\plugins.ps1" > $null
	Add-Symlink "${mydocs}\powershell\aliases.ps1" "$DotFilesDirectory\app\df\powershell\aliases.ps1" > $null
	Add-Symlink "${mydocs}\powershell\hotkeys.ps1" "$DotFilesDirectory\app\df\powershell\hotkeys.ps1" > $null
	Add-Symlink "${mydocs}\powershell\wsl.ps1" "$DotFilesDirectory\app\df\powershell\wsl.ps1" > $null
	Add-Symlink "${mydocs}\powershell\_functions.ps1" "$DotFilesDirectory\app\df\powershell\_functions.ps1" > $null
	Add-Symlink "${mydocs}\Powershell\powershell.config.json" "$DotFilesDirectory\app\df\powershell\powershell.config.json" > $null
...
#################################################################################
	...
	#Add-Symlink "$Env:APPDATA\Code\User\snippets" "$DotFilesDirectory\app\df\vscode\snippets" > $null
...
#################################################################################
		...
		if (-not [string]::IsNullOrWhiteSpace($windowsTerminalSettingsPath)) {
			Add-Symlink $windowsTerminalSettingsPath "$DotFilesDirectory\app\df\windows-terminal\settings.json" > $null
		}
...
#################################################################################
	if (-not ($RefreshDotfiles -or $RelinkOnly)) {
		Install-FontWindows("$DotFilesDirectory\app\df\assets\fonts\comic-code-nerdfonts")
	}
