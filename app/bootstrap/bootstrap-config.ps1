...
function Write-BootstrapConfigPathPreferenceWarnings {
	...
	Write-Warning 'Ainda existem caminhos relativos no app/bootstrap/user-config.yaml. O padrao recomendado agora e usar caminhos absolutos e canonicos.'
...
function Sync-BootstrapDerivedFiles {
	...
	$secretsRef = @(
		'---'
		'# Secrets do repositório dotfiles (gerado a partir de app/bootstrap/user-config.yaml)'
		'1password:'
...
function Ensure-BootstrapConfigReady {
	...
	$templatePath = Join-Path $DotFilesDirectory 'app\bootstrap\user-config.yaml.tpl'
	$configPath = Join-Path $DotFilesDirectory 'app\bootstrap\user-config.yaml'
