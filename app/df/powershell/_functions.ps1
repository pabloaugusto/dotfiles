...
function checkEnv {
	...
	if ($IsWindows) {
		...
		if ($oneDriveEnabled) {
			...
			if ($resolvedRoot) {
				...
				Add-CheckResult -Item 'OneDrive root path' -Status 'fail' -Detail 'Unable to resolve an existing OneDrive root path.' -Solution 'Configure OneDrive or set paths.windows.onedrive_root in app/bootstrap/user-config.yaml.'
			}
	...
	if (Test-CommandExists op) {
		...
		if ([string]::IsNullOrWhiteSpace($currentRepoRoot)) {
			$secretsRefPath = Join-Path $Env:USERPROFILE 'dotfiles\app\df\secrets\secrets-ref.yaml'
		}
		else {
			$secretsRefPath = Join-Path $currentRepoRoot 'app\df\secrets\secrets-ref.yaml'
		}
...
function Get-DotfilesSyncContext {
	...
	switch -Regex ($normalized) {
		...
		'^app/bootstrap/' { return 'bootstrap' }
		'^app/df/powershell/' { return 'powershell' }
		'^app/df/bash/' { return 'bash' }
		'^app/df/zsh/' { return 'zsh' }
		'^app/df/git/' { return 'git' }
		'^app/df/ssh/' { return 'ssh' }
		'^docs/' { return 'docs' }
